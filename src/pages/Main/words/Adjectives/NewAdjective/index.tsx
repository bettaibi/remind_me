...
import { v4 } from 'uuid';
import { addAdjective } from '../../../../../store/actions/adjectives.actions';
...
export const NewAdjective: React.FC<AddProps> = ({ handleToogle, saveByKey }) => {
    ...
    const create = async (values: AdjectiveModel, resetForm: ()=> void) => {
        try {
            const id = v4();
            const res = await saveByKey({ ...values, id }, id);
            if (res.success) {
                showMsg('created', res.message);
                dispatch(addAdjective({ ...values, id }));
                resetForm();
            }
            else {
